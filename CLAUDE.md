...
### 4단계: 빠르게 출시, 반복
- 템플릿으로 공통 인프라(Firebase, AdMob, GDPR) 코드는 이미 완성되어 있음
- Firebase 프로젝트 생성 + google-services.json 배치 + AdMob ID 교체만 하면 됨
- 앱 고유 기능 개발에만 집중할 수 있는 구조
...
- **앱마다** 비공개 테스트 14일 + 테스터 12명 이상 충족해야 프로덕션 출시 가능
- SpickaRoo 프로덕션 출시 완료 (2026-03-26)해도 다른 앱의 14일 조건은 면제되지 않음
- 모든 앱에 대해 비공개 테스트 → 14일 대기 → 테스터 12명 확보 → 프로덕션 신청 과정 필요
...
## 메모리 업데이트 규칙 — 앱 완성 후 반드시 수행

앱 하나를 만들거나 수정할 때마다, 세션 종료 전에 아래 메모리를 업데이트해야 한다.

### 1. 앱 포트폴리오 업데이트 (`project_10apps_status.md`)
- 새 앱 추가 시: 테이블에 행 추가 (앱명, 패키지, 카테고리, 설명, 핵심 패키지, 상태)
- 기존 앱 수정 시: 해당 행의 상태/설명 업데이트
- 카테고리 분포 섹션도 갱신

### 2. 삽질 기록 추가 (이 파일의 `삽질 기록 & 해결 패턴` 섹션)
- 개발 중 겪은 문제와 해결법을 번호 이어서 추가
- 형식: `#### N. 제목` → `문제` → `해결`
- 같은 실수를 반복하지 않기 위한 기록이므로, 사소해 보여도 기록

### 3. 전략/피드백 업데이트 (해당 시)
- 새로운 시장 전략이나 틈새 공략 사례 → `project_strategy.md` 업데이트
- 사용자가 워크플로우 수정을 요청하면 → `feedback_dev_workflow.md` 업데이트

### 업데이트 타이밍
- **앱 개발 완료 후** (Initial commit 직후)
- **버그 수정/개선 후** (삽질이 있었던 경우)
- **세션 종료 전** (사용자가 "자자" 등으로 마무리할 때)
